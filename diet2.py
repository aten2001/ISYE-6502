# -*- coding: utf-8 -*-
"""
Created on Tue Apr  2 21:49:00 2019
...
# The daily constraints are added to 'prob' for 15.2.1
...
#This decision variable represent whether we eat a food
chosenVars = LpVariable.dicts("Chosen",Ingredients,0,1,LpBinary)

#Other constraints from 15.2.2 a, b, c
#Constraints for 15.2.2a set upperbound to link amount and binary variables first
for i in Ingredients:
    prob += ingredient_vars[i] <= 10000000*chosenVars[i]
#set lowerbound for amount variable
for i in Ingredients:
    prob += ingredient_vars[i] >= 0.1*chosenVars[i]

#Constraint for 15.2.2b
prob += chosenVars['Celery, Raw'] + chosenVars['Frozen Broccoli'] <= 1.0

#Constraint for 15.2.2c
prob += chosenVars['Ham,Sliced,Extralean'] + chosenVars['Frankfurter, Beef']
+ chosenVars['Hamburger W/Toppings'] + chosenVars['Hotdog, Plain']
+ chosenVars['Scrambled Eggs'] + chosenVars['Kielbasa,Prk']
+ chosenVars['Poached Eggs'] + chosenVars['Pork']
+ chosenVars['Roasted Chicken'] + chosenVars['Sardines in Oil']
+ chosenVars['White Tuna in Water'] >=3.0
